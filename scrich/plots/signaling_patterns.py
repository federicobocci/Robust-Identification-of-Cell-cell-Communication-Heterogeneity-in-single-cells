...
def pattern_marker(ax, adata, pattern, ntop=25, human=True, cmap=plt.cm.tab20,
    ...
    path_list = adata.uns['selected_pathways']
    if human:
        lig = pd.read_csv('pathway_database/receptor_human.csv')
        rec = pd.read_csv('pathway_database/receptor_human.csv')
    else:
        lig = pd.read_csv('pathway_database/ligand_mouse.csv')
        rec = pd.read_csv('pathway_database/receptor_mouse.csv')
